...
conf = {
    ...
    "save_path": "Gcnn_v11_test/version_2/",
    "output_path":"Gcnn_v11_output/version_2/",
    # "word_emb_init": "./data/word_embedding.pkl",
    "init_model": "Gcnn_v11_model/version_2/",  # should be set for test
...
class MyModel(object):
    ...
    #    self.keep_prob = tf.placeholder(tf.float32)
        ...
        sentence_GRU_2 = tf.nn.rnn_cell.GRUCell(self.rnn_units_2, kernel_initializer=tf.orthogonal_initializer())
     ...
     #   A_matrix = tf.get_variable('A_matrix_v', shape=( self.dim ,  self.dim ),
      ...
      #  B_matrix = tf.get_variable('B_matrix_v', shape=(self.dim*4, self.dim*4),
       ...
       # input_hyp = emb(self.hypothesis_x, mask_hyp)  #response_embeddings
        ...
        #   conv1_hyp:(64, 50, 300)
        ### Attention ###
            self.utter_bi = utterance_GRU_embeddings  # (64, 50, 200)
           # print(conv1_utter.shape) #
            self.response_bi = response_GRU_embeddings
        ...
        # hypothesis_attns:[64 50 200]
          ...
          #  print('hypothesis_attns:', response_attns.get_shape().as_list())  # [None, hyp_len, 600]
            ...
            m_a = tf.concat([self.utter_bi, utter_attns, utter_diff, utter_mul],2)  # premise_attns：[64 50 200] m_a：[None, prem_len, 4*200]
            ...
            infer_utter, _ = tf.nn.dynamic_rnn(sentence_GRU_2, m_a,
                                                            sequence_length=utterance_len, dtype=tf.float32,
                                                            scope='sentence_GRU_2')
          #  infer_reaponse = conv_encoder_stack(m_b, nhids_list=[200] * conf["word_layers_agg"],
           #                                     kwidths_list=[3] * conf["word_layers_agg"], dropout_dict=dropout_dict,
           #                                     var_scope_name="inference_", reuse=True)
            infer_reaponse, _ = tf.nn.dynamic_rnn(sentence_GRU_2, m_b,
                                               sequence_length=utterance_len, dtype=tf.float32,
                                               scope='sentence_GRU_2')
       ...
       # self.matching_vectors=matching_vectors
        #self.b = tf.stack(matching_vectors, axis=1, name='matching_stack') #[10 64  800]
       ...
       #                                  kwidths_list=[3] * conf["word_layers_itg"], dropout_dict=dropout_dict,
        ...
        _, last_hidden = tf.nn.dynamic_rnn(final_GRU, tf.stack(matching_vectors, axis=0, name='matching_stack'),
                                                dtype=tf.float32,
                                                time_major=True, scope='final_GRU')
        #[64 10 200]
      ...
      #  conv_2 = tf.nn.dropout(conv_2, keep_prob=self.keep_prob)
       # self.logits = tf.contrib.layers.fully_connected(inputs=conv_2, num_outputs=2, activation_fn=None,weights_initializer=tf.contrib.layers.xavier_initializer(),biases_initializer=tf.zeros_initializer())
        ...
        self.logits = tf.layers.dense(last_hidden, 2, kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                      name='final_v')
