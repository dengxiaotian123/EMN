...
conf = {
    ...
    "save_path": "Gcnn_v14_test/version/",
    "output_path":"Gcnn_v14_output/version/",
    # "word_emb_init": "./data/word_embedding.pkl",
    "init_model": "Gcnn_v14_model/version/",  # should be set for test
     ...
     #   start=time.time
      ...
      #  B_matrix = tf.get_variable('B_matrix_v', shape=(self.dim*4, self.dim*4),
       #                            initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
        ...
        # hypothesis_attns:[64 50 200]
          ...
          #  print('hypothesis_attns:', response_attns.get_shape().as_list())  # [None, hyp_len, 600]
            ...
            conv_layer = tf.layers.conv2d(matrix, filters=self.filter_size, kernel_size=shape, padding='VALID',
                                          ...
                                          activation=tf.nn.relu, reuse=reuse_t, name='conv1')  # TODO: check other params
            pooling_layer = tf.layers.max_pooling2d(conv_layer, (3, 3), strides=(3, 3),
                                                    padding='VALID', name='pooling1')  # TODO: check other params
            #tf.contrib.layers.flatten(pooling_layer)[64 16*16*filter_size] filter_size必须大于4
            outdim=v_2_max.get_shape().as_list()[-1]
            matching_vector = tf.layers.dense(tf.contrib.layers.flatten(pooling_layer), outdim,
                                              kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                              activation=tf.tanh, reuse=reuse_t,
                                              name='matching_v')  # TODO: check wthether this is correct
             # v_1_ave：[batch_size  hidden_dim]
           # mx = tf.stack([v, matching_vector], axis=2)#[64 400 2]
            v = tf.concat([v_1_ave, v_2_ave, v_1_max, v_2_max, matching_vector], 1)
           # mx = tf.concat([v, matching_vector], 1) #[64 1000]
        ...
        self.logits = tf.contrib.layers.fully_connected(inputs=last_hidden, num_outputs=2, activation_fn=None,weights_initializer=tf.contrib.layers.xavier_initializer(),biases_initializer=tf.zeros_initializer())
      ...
      #  self.learning_rate = tf.train.exponential_decay(0.001, global_step=self.global_step, decay_steps=800,
