...
def dot_sim(x, y, is_nor=True):
    ...
    sim = tf.einsum('bik,bjk->bij', x, y) #Batch matrix multiplication
...
def dense(x, out_dimension=None, add_bias=True):
    ...
    if out_dimension is None:
        out_dimension = x.shape[-1] #如果没有设置输出的维度，就设置为输入的最后数据的一维
...
def positional_encoding_vector(x, min_timescale=1.0, max_timescale=1.0e4, value=0):
    '''Adds a bunch of sinusoids(sin wave) of different frequencies to a tensor.
...
def mask(row_lengths, col_lengths, max_row_length, max_col_length):
    ...
    col_mask = tf.cast(tf.expand_dims(col_mask, -1), tf.float32) #数据类型转换：bool转换为float32
...
def weighted_sum(weight, values):
    ...
    Raises:
    '''
    return tf.einsum('bij,bjk->bik', weight, values) #矩阵相乘
