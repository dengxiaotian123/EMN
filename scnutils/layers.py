...
def loss(x, y, num_classes=2, is_clip=True, clip_value=10):
    ...
    logits = tf.reshape(tf.matmul(x, W) + bias, [-1]) #单层感知机
...
def attention(
    ...
    Q_lengths, K_lengths, 
    attention_type='dot', 
    is_mask=True, mask_value=-2**32+1,#-4294967295
    drop_prob=None):
    '''Add attention layer.
    Args:
    ...
    if attention_type == 'dot':
        logits = op.dot_sim(Q, K) #[batch, Q_time, time][256 50 50]
    if attention_type == 'bilinear':
        logits = op.bilinear_sim(Q, K)

    if is_mask:
        mask = op.mask(Q_lengths, K_lengths, Q_time, K_time) #[batch, Q_time, K_time][256 50 50]
        logits = mask * logits + (1 - mask) * mask_value  #[batch, Q_time, K_time][256 50 50]
    
    attention = tf.nn.softmax(logits)  #[batch, Q_time, K_time][256 50 50]

    if drop_prob is not None:
        print('use attention drop')
        attention = tf.nn.dropout(attention, drop_prob)

    return op.weighted_sum(attention, V) #[256 50 200]
...
def CNN_3d(x, out_channels_0, out_channels_1, add_relu=True):
    ...
    Raises:
    '''
    in_channels = x.shape[-1] #12个通道
    weights_0 = tf.get_variable(
        name='filter_0',
        shape=[3, 3, 3, in_channels, out_channels_0], #out_channels_0=32
        dtype=tf.float32,
        initializer=tf.random_uniform_initializer(-0.01, 0.01))
    ...
    weights_1 = tf.get_variable(
        name='filter_1',
        shape=[3, 3, 3, out_channels_0, out_channels_1], #out_channels_1=16
        dtype=tf.float32,
        initializer=tf.random_uniform_initializer(-0.01, 0.01))
    ...
    return tf.contrib.layers.flatten(pooling_1) #[256,576]
