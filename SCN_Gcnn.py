...
conf = {
    ...
    "save_path": "Gcnn_test/",
    "output_path":"Gcnn_output/",
    # "word_emb_init": "./data/word_embedding.pkl",
    "init_model": "Gcnn_model/",  # should be set for test
       ...
       # with self._graph.as_default():
        ...
        #utterance_embeddings 10个[40 50 200],utterance_len 10个[40]
           ...
           # shape=(self.filter_h,self.filter_h)
            ...
            h, res_input = matrix, matrix
            fanin_depth = matrix.get_shape()[-1]
            shape = (self.filter_h, self.filter_h, fanin_depth, self.filter_size)  # self.filter_size=8 原来kernel_size是（3,3）
            with tf.variable_scope("GCNN", reuse=reuse):
                conv_w = self.conv_op(h, shape, "linear")
                conv_v = self.conv_op(h, shape, "gated")
                h = conv_w * tf.sigmoid(conv_v)
                conv_layer = h+res_input
      ...
      #  val_batch_num = len(val_batches["response"])
        ...
        with tf.Session() as sess:
            ...
            for step_i in xrange(conf["epoch"]):
                ...
                if step_i > 3 and (step_i+1)%2==0: #模型保存6 8 10
