...
import scnutils.evaluation as eva
...
conf = {
    ...
    "save_path": "Gcnninf_test/version_1",
    "output_path":"Gcnninf_output/version_1",
    # "word_emb_init": "./data/word_embedding.pkl",
    "init_model": "Gcnninf_model/version_1",  # should be set for test
...
class MyModel(object):
 ...
 #               inputs=tf.nn.conv1d(value=inputs, filters=V, stride=1, padding=padding) # [64 50 900] #-inf
        ...
        A_matrix = tf.get_variable('A_matrix_v', shape=(4* self.dim , 4* self.dim ),
       ...
       # input_hyp = emb(self.hypothesis_x, mask_hyp)  #response_embeddings
        ...
        #   conv1_hyp:(64, 50, 300)
           # self.look1=input_conv_1
           # self.look2=next_layer_1
           # self.look3=inputs_look_1
          #  self.look4 = V_1
        ### Attention ###
          #  self.utter_bi = conv1_utter  # (64, 50, 200)
           # print(conv1_utter.shape) #
          #  self.response_bi = conv1_response
        # self.scores_unnorm =tf.matmul(self.premise_bi/tf.sqrt(tf.norm(self.premise_bi,ord='euclidean',axis=[-2,-1],keep_dims=True)),self.hypothesis_bi/tf.sqrt(tf.norm(self.hypothesis_bi,ord='euclidean',axis=[-2,-1],keep_dims=True)),transpose_a=False,transpose_b=True)
        ...
        # self.betas:[64 50 50]
          #  response_expand = tf.tile(tf.expand_dims(self.response_bi, 1),[1, self.max_sentence_len, 1, 1])  # (batch_size,prem_len,hyp_len,hidden_dim)
        ...
        # hypothesis_attns:[64 50 200]
         ...
         #   utter_mul = tf.multiply(self.utter_bi, utter_attns)
          ...
          #  m_b = tf.concat([self.response_bi, response_attns, response_diff, response_mul], 2)  # 各种维度整合的方式 #[64 50 800]
            matrix1 = tf.matmul(utterance_embeddings, response_embeddings_T)
          #  print(m_a.shape)
            matrix2 = tf.einsum('aij,jk->aik', conv1_utter, A_matrix)  # TODO:check this #A_matrix[800 800]
            matrix2 = tf.matmul(matrix2, tf.transpose(conv1_response,perm=[0,2,1]))
            ...
            conv_layer = tf.layers.conv2d(matrix, filters=self.filter_size, kernel_size=shape, padding='VALID',
                                          ...
                                          activation=tf.nn.relu, reuse=reuse_t, name='conv')  # TODO: check other params
            ...
            matching_vector = tf.layers.dense(tf.contrib.layers.flatten(pooling_layer), 50,
                                              ...
                                              activation=tf.tanh, reuse=reuse_t,
                                              name='matching_v')  # TODO: check wthether this is correct
            if not reuse_t:
                reuse_t = True
            matching_vectors.append(matching_vector)
        self.b = tf.stack(matching_vectors, axis=0, name='matching_stack')  # [10 80 50]
        #print('b', b.get_shape().as_list())  #
        _, self.last_hidden = tf.nn.dynamic_rnn(final_GRU, tf.stack(matching_vectors, axis=0, name='matching_stack'),
                                                dtype=tf.float32,
                                                time_major=True, scope='final_GRU')  # TODO: check time_major
        self.logits = tf.layers.dense(self.last_hidden, 2, kernel_initializer=tf.contrib.layers.xavier_initializer(),name='final_v')
        ...
        with tf.Session() as sess:
            ...
            for step_i in xrange(conf["epoch"]):
                ...
                for batch_index in range(batch_num):
                  ...
                  #  utter_bi = sess.run(self.utter_bi, feed_dict=feed_dict)
                  #  print('utter_bi', utter_bi)
                  #  logits = sess.run(self.logits, feed_dict=feed_dict)
                  #  print('logits', logits)
                  #  y_pred = sess.run(self.y_pred, feed_dict=feed_dict)
                  #  print('y_pred', y_pred)

                   # total_loss=sess.run(self.total_loss,feed_dict=feed_dict)
