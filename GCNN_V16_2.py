...
class MyModel(object):
    def __init__(self, conf):
        ...
        self.max_turn_num=conf["max_turn_num"]   #10
      ...
      #  self.batch_size = conf['batch_size']
        ...
        final_GRU = tf.nn.rnn_cell.GRUCell(200, kernel_initializer=tf.orthogonal_initializer())
        ...
        for utterance_embeddings, utterance_len ,utterance_ph in zip(all_utterance_embeddings, all_utterance_len, all_utterance_ph):
            ...
            conv1_utter = conv_encoder_stack(utterance_embeddings, nhids_list=[100] * conf["word_layers_enc"],
                                        kwidths_list=[3] * conf["word_layers_enc"], dropout_dict=dropout_dict,
                                        var_scope_name="encoder_", reuse=reuse_t)

            #self.conv=conv1_utter
            conv1_response = conv_encoder_stack(response_embeddings, nhids_list=[100] * conf["word_layers_enc"],
                                       kwidths_list=[3] * conf["word_layers_enc"], dropout_dict=dropout_dict,
        ...
        # hypothesis_attns:[64 50 200]
          ...
          #  print('hypothesis_attns:', response_attns.get_shape().as_list())  # [None, hyp_len, 600]
            ...
            v1 = tf.concat([v_1_ave, v_2_ave, v_1_max, v_2_max], 1)  # v_1_ave：[64  400]
            matching_vectors.append(v1)

            if not reuse_t:
                reuse_t = True
        self.matching_vectors = matching_vectors  # [64 10 400]
        self.b = tf.stack(matching_vectors, axis=1, name='matching_stack')  # [64 10 400]
        self.conv2_itg = conv_encoder_stack(self.b, nhids_list=[80] * conf["word_layers_itg"],
                                            kwidths_list=[3] * conf["word_layers_itg"], dropout_dict=dropout_dict,
                                            var_scope_name="integration_", reuse=None)
        # [64 10 80]
        # = self.conv2_itg.get_shape().as_list() #[64 10 40]
        conv_2 = tf.contrib.layers.flatten(self.conv2_itg)
        # conv_2=tf.contrib.layers.fully_connected(inputs=conv2, num_outputs=200, activation_fn=None,weights_initializer=tf.contrib.layers.xavier_initializer(),biases_initializer=tf.zeros_initializer())
        # conv_2  [64 400]
        print('con_v',conv_2.get_shape().as_list())
        ...
        rnn_output2, _ = tf.nn.dynamic_rnn(sentence_GRU, s_r_emb,
                                           sequence_length=self.s_r_len, dtype=tf.float32,
                                           scope='sentence_GRU')
        rnn_output1, _ = tf.nn.dynamic_rnn(sentence_GRU, s_turn_emb,
                                           sequence_length=self.single_turn_len, dtype=tf.float32,
                                           scope='sentence_GRU')
        dropout_dict = {'src': 1.0, 'hid': 1.0}

        # [batch_size, question_len, dim]
        question_output = rnn_output1
        # [batch_size, answer_len, dim]
        answer_output = rnn_output2
        self.scores_unnorm = tf.matmul(question_output, answer_output, transpose_a=False, transpose_b=True)
        # tf.matmul 矩阵相乘 第一个矩阵的列数（column）等于第二个矩阵的行数（row）[64 50 50]
        self.scores_unnorm_exp = tf.exp(self.scores_unnorm)
        self.alphas = masked_attention_axis2(self.scores_unnorm_exp, tf.transpose(s_mask_response, perm=[0, 2,
                                                                                                       1]))  # (batch_size,prem_len,hyp_len)
        # self.alphas:[64 50 50]
     ...
     #   rnn_size_layer_2 = conf["rnn_size"]
        ...
        #  rnn_scope_layer2, scope_reuse=False)
        ...
        rnn_output_q_1, rnn_states_q_1 = tf.nn.dynamic_rnn(final_GRU, m_a, dtype=tf.float32, scope='final_GRU')
        rnn_output_a_2, rnn_states_a_2 = tf.nn.dynamic_rnn(final_GRU, m_b, dtype=tf.float32, scope='final_GRU')

        question_output_2 = rnn_output_q_1
        answer_output_2 = rnn_output_a_2
        v1_bi = question_output_2 * s_mask_utter  # mask: (64, 200, 1) #padding 的地方归零v1_bi [64 50 300]
        v2_bi = answer_output_2 * s_mask_response

        v_1_sum = tf.reduce_sum(v1_bi, 1)  # v1_bi [batch_size prem_len hidden_dim] v_1_sum=[batch_size hidden_dim]
        v_1_ave = tf.div(v_1_sum, tf.expand_dims(tf.cast(self.single_turn_len, tf.float32) + (1e-10),
                                                 -1))  # [batch_size  hidden_dim]

        v_2_sum = tf.reduce_sum(v2_bi, 1)
        v_2_ave = tf.div(v_2_sum, tf.expand_dims(tf.cast(self.s_r_len, tf.float32) + (1e-10),
                                                 -1))  # [batch_size prem_len hidden_dim]

        v_1_max = tf.reduce_max(v1_bi, 1)
        v_2_max = tf.reduce_max(v2_bi, 1)

        v2 = tf.concat([v_1_ave, v_2_ave, v_1_max, v_2_max], 1)  # v_1_ave：[64  800]

        print('v2:', v2.get_shape().as_list())
        conv_2 = tf.concat([conv_2,v2],axis=1)#[64 1600]
        print('conv_2:',conv_2.get_shape().as_list())
        self.logits = tf.contrib.layers.fully_connected(inputs=conv_2, num_outputs=2, activation_fn=None,weights_initializer=tf.contrib.layers.xavier_initializer(),biases_initializer=tf.zeros_initializer(),weights_regularizer=tf.contrib.layers.l2_regularizer(0.001))
        #self.logits = tf.layers.dense(conv_2, 2, kernel_initializer=tf.contrib.layers.xavier_initializer(),name='final_v')
        reg = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
        loss_L2 = tf.add_n(reg)
        self.y_pred = tf.nn.softmax(self.logits)
        self.total_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.y_true, logits=self.logits ))+loss_L2
        tf.summary.scalar('loss', self.total_loss)
        self.global_step = tf.Variable(0, name="global_step", trainable=False)
        self.learning_rate = tf.train.exponential_decay(0.0005, global_step=self.global_step, decay_steps=1000,
        ...
        with tf.Session() as sess:
            ...
            for step_i in xrange(conf["epoch"]):
                ...
                for batch_index in range(batch_num):
                  #  print(np.array(train_batches["single_turn"][batch_index]))
                  #  print(np.array(train_batches["single_turn_len"][batch_index]))
                  #  print(np.array(train_batches["single_r"][batch_index]))
                  #  print(np.array(train_batches["single_r_len"][batch_index]))
