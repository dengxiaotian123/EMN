...
conf = {
    ...
    "save_path": "Gcnn_v2_test/version1/",
    "output_path":"Gcnn_v2_output/version1/",
    # "word_emb_init": "./data/word_embedding.pkl",
    "init_model": "Gcnn_v2_model/version1/",  # should be set for test
    "embedding_file": "../../data/ubuntu/word_embedding.pkl",
    "CPU":"/cpu:0", #'/gpu:1'

    "emb_train":False,
    "word_embedding_dim":200,
    "batch_size": 80,  # 200 for test
    "epoch":5,
    "max_turn_num": 10,
    "max_turn_len": 50,
       ...
       # input_hyp = emb(self.hypothesis_x, mask_hyp)  #response_embeddings
        ...
        # hypothesis_attns:[64 50 200]
          ...
          #  print('hypothesis_attns:', response_attns.get_shape().as_list())  # [None, hyp_len, 600]
            ...
            matrix1 = tf.matmul(utterance_embeddings, response_embeddings_T) #utterance_embeddings[64 50 200]
            print(m_a.shape)
            matrix2 = tf.einsum('aij,jk->aik', infer_utter, A_matrix)  # TODO:check this #A_matrix[800 800]
            matrix2 = tf.matmul(matrix2, tf.transpose(infer_reaponse,perm=[0,2,1]))
            matrix3 = tf.einsum('aij,jk->aik', m_a, B_matrix)  # TODO:check this #A_matrix[800 800]
            matrix3 = tf.matmul(matrix3, tf.transpose(m_b, perm=[0, 2, 1]))
            matrix = tf.stack([matrix1, matrix2, matrix3], axis=3, name='matrix_stack') #[64 50 50 2]
