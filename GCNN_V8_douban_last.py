...
conf = {
    ...
    "save_path": "Gcnn_v8last_test/version/",
    "output_path":"Gcnn_v8last_output/version/",
    # "word_emb_init": "./data/word_embedding.pkl",
    "init_model": "Gcnn_v8last_model/version/",  # should be set for test
     ...
     #   A_matrix = tf.get_variable('A_matrix_v', shape=( self.dim ,  self.dim ),
      ...
      #  B_matrix = tf.get_variable('B_matrix_v', shape=(self.dim*4, self.dim*4),
       ...
       # input_hyp = emb(self.hypothesis_x, mask_hyp)  #response_embeddings
        ...
        #[64 10 200]
    #    conv_2 = tf.nn.dropout(self.last_hidden, keep_prob=self.keep_prob)
        self.logits = tf.layers.dense(self.last_hidden, 2, kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                      name='final_v')
