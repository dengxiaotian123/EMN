...
conf = {
    ...
    "save_path": "Gcnn_v13_test/version/",
    "output_path":"Gcnn_v13_output/version/",
    # "word_emb_init": "./data/word_embedding.pkl",
    "init_model": "Gcnn_v13_model/version/",  # should be set for test
     ...
     #   A_matrix = tf.get_variable('A_matrix_v', shape=( self.dim ,  self.dim ),
      ...
      #  B_matrix = tf.get_variable('B_matrix_v', shape=(self.dim*4, self.dim*4),
       ...
       # input_hyp = emb(self.hypothesis_x, mask_hyp)  #response_embeddings
        ...
        conv_2=tf.contrib.layers.flatten(self.b)

        self.logits = tf.contrib.layers.fully_connected(inputs=conv_2,
                                                        num_outputs=2,
                                                        activation_fn=None,
