...
conf = {
    ...
    "save_path": "Gcnn_v3_test/version4/",
    "output_path":"Gcnn_v3_output/version4/",
    # "word_emb_init": "./data/word_embedding.pkl",
    "init_model": "Gcnn_v3_model/version4/",  # should be set for test
...
class MyModel(object):
    ...
    ## Functions
     ...
     #   A_matrix = tf.get_variable('A_matrix_v', shape=( self.dim ,  self.dim ),
      ...
      #  B_matrix = tf.get_variable('B_matrix_v', shape=(self.dim*4, self.dim*4),
       ...
       # input_hyp = emb(self.hypothesis_x, mask_hyp)  #response_embeddings
        ...
        # FIXED_PARAMETERS["word_layers_agg"]=2
         #   self.emb_utter=utterance_embeddings
            conv1_utter = conv_encoder_stack(utterance_embeddings, nhids_list=[200] * conf["word_layers_enc"],
                                        kwidths_list=[2] * conf["word_layers_enc"], dropout_dict=dropout_dict,
        ...
        # hypothesis_attns:[64 50 200]
          ...
          #  print('hypothesis_attns:', response_attns.get_shape().as_list())  # [None, hyp_len, 600]
            ...
            infer_utter = conv_encoder_stack(m_a, nhids_list=[200] * conf["word_layers_agg"],
                                             kwidths_list=[2] * conf["word_layers_agg"], dropout_dict=dropout_dict,
        ...
        self.conv2_itg = conv_encoder_stack(self.b, nhids_list=[100] * conf["word_layers_itg"],
                                         kwidths_list=[2] * conf["word_layers_itg"], dropout_dict=dropout_dict,
      ...
      #  conv_2 = tf.nn.dropout(conv_2, keep_prob=self.keep_prob)
        ...
        logits = tf.contrib.layers.fully_connected(inputs=conv_2, num_outputs=2, activation_fn=None,weights_initializer=tf.contrib.layers.xavier_initializer(),biases_initializer=tf.zeros_initializer())
     ...
     #   self.y_pred = tf.nn.softmax(self.logits)
        self.logits = tf.matmul(logits, W) + bias
        self.logits = tf.squeeze(logits, [1])
       # self.y_pred = tf.nn.softmax(self.logits)
        self.y_pred = tf.sigmoid(self.logits, name="prob")
       # losses = tf.nn.sigmoid_cross_entropy_with_logits(logits=(self.logits), labels=self.y_true)
        self.total_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.y_true, logits=self.logits ))
        tf.summary.scalar('loss', self.total_loss)
        global_step = tf.Variable(0, name="global_step", trainable=False)
        self.learning_rate = tf.train.exponential_decay(self.lr, global_step, 5000, 0.96, staircase=True)
    ...
    def Evaluate(self,sess,val_batches,score_file_path):
       ...
       # eva_score_file = open(score_file_path, 'w')
        for batch_index in xrange(val_batch_num):
          ...
          #  print('val_loss',val_loss)
            ...
            self.all_candidate_scores.append(candidate_scores)
       ...
       # eva_score_file.close()
        ...
        R10_5=Evaluate.ComputeR10_5(all_candidate_scores, labels)
        ...
        with tf.Session() as sess:
            ...
            for step_i in xrange(conf["epoch"]):
                ...
                for batch_index in range(batch_num):
                    ...
                    _, learning_rate,summary = sess.run([self.train_op, self.learning_rate,merged], feed_dict=feed_dict)
                    ...
                    if step % conf["print_step"] == 0 and step > 0:  # print_step=125 一个epoch打印100次
                        print('epoch={i}'.format(i=step_i+1), 'step:', step, "loss",
                             ...
                             "processed: [" + str(step * 1.0 / batch_num) + "]",'learning_rate',learning_rate)
                    if step % conf["evaluate_step"] == 0 and step > 0:  # 12500的倍数就会打印
                        ...
                        valid_mrr=self.Evaluate(sess, val_batches, score_file_path)
                        if valid_mrr > best_mrr:
                            best_mrr = valid_mrr
                            path=saver.save(sess, os.path.join(conf["init_model"], "model.{0}".format(step_i + 1)))
