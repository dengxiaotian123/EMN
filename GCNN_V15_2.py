...
class MyModel(object):
   ...
   # def BuildModel(self):
     ...
     #   self.lr = tf.placeholder(tf.float32)
        ...
        A_matrix = tf.get_variable('A_matrix_v', shape=( 800,  800 ),
      ...
      #  B_matrix = tf.get_variable('B_matrix_v', shape=(self.dim*4, self.dim*4),
       #                            initializer=tf.contrib.layers.xavier_initializer(), dtype=tf.float32)
        ...
        # hypothesis_attns:[64 50 200]
          ...
          #  print('hypothesis_attns:', response_attns.get_shape().as_list())  # [None, hyp_len, 600]
           ...
           # v = tf.concat([v_1_ave, v_2_ave, v_1_max, v_2_max, matching_vector], 1)
            mx = tf.concat([v, matching_vector], 1) #[64 1600]
            matching_vectors.append(mx)
            if not reuse_t:
                reuse_t = True
        #[64 10 1600]
        ...
        self.logits = tf.contrib.layers.fully_connected(inputs=last_hidden, num_outputs=2, activation_fn=None,weights_initializer=tf.contrib.layers.xavier_initializer(),biases_initializer=tf.zeros_initializer(),weights_regularizer=tf.contrib.layers.l2_regularizer(0.001))
